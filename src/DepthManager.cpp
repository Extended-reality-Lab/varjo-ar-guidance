...
DepthManager::DepthManager(varjo_Session* session, const std::function<void(const Frame&)>& onFrameCallback)
 ...
 stereo = cv::StereoSGBM::create(storedSGBMData.minDisp, storedSGBMData.numDisparity, storedSGBMData.blockSize,
        storedSGBMData.p1, storedSGBMData.p2, storedSGBMData.disp12MaxDiff,
        storedSGBMData.preFilterCap, storedSGBMData.uniquenessRatio,
        storedSGBMData.speckleWindowSize, storedSGBMData.speckleRange, false);
}
...
// Calculate rectification maps for stereo images
void DepthManager::getUndistortMapDat(float w, float h) {
    ...
    // Outputs map1L, map2L, map1R, map2R (for use in remap())
...
// Calculate depth map using SGBM algorithm
